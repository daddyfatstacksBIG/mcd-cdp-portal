...
const CDPCreateConfirmSummary = ({
  ...
  const {liquidationPenalty, liquidationRatio, stabilityFee} = selectedIlk.data;
  const {gemsToLock, daiToDraw} = cdpParams;
  const {liquidationPrice, collateralizationRatio} =
  ...
  css = "white-space: nowrap"
        ...
        >
          ...
          />
    <Text color = "grey.500" ml = "s"> {lang.formatString(
          ...
          <Checkbox
  checked = {hasUnderstoodSF} onChange =
  {
    () => setHasUnderstoodSF(state => !state)
  } />
    ...
    (async () => {
      ...
      const waitTimeText =
          waitTime < 1
              ...
              : `${minutes} ${
                    minutes === 1 ? lang.cdp_create.minutes_wait_time_singular
...
const CDPCreateConfirmCDP = ({dispatch, cdpParams, selectedIlk, onClose}) => {
  ...
  async function capturedDispatch(payload) {
    ...
    const txObject =
        maker.service('mcd:cdpManager')
            .openLockAndDraw(selectedIlk.key, selectedIlk.currency(gemsToLock),
                             daiToDraw);
